...
const _lib = { ...baileys, ...(typeof baileys.default === 'object' ? baileys.default : {}) }

const _makeWaSocket = typeof baileys.default === 'function' ? baileys.default : (_lib.makeWASocket || _lib.default)
const proto = _lib.proto || _lib.WAProto
const {
    makeWALegacySocket,
...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendButton: {
            async value(jid, title, body, footer, button = [], m, options = {}, smlcap = { smlcap: false }) {
...
let pathFile1 = await writeExif(media, { packname: options.packname ? options.packname : 'BOBIZA -BOT', author: options.author ? options.author : '', categories: options.categories ? options.categories : [] })
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        /*text: {
            ...
            get() {
                const msg = this.msg
                const text = (typeof msg === 'string' ? msg : msg?.text) || msg?.caption || msg?.contentText || msg?.selectedId || msg?.nativeFlowResponseMessage || msg?.message?.imageMessage?.caption || msg?.message?.videoMessage?.caption || ''
                return typeof this._text === 'string' ? this._text : '' || (typeof text === 'string' ? text : (
                    text?.selectedDisplayText ||
                    text?.hydratedTemplate?.hydratedContentText ||
